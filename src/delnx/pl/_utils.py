...
def matrix(
    ...
    *,
    ...
    plt.colorbar(img, shrink=colorbar_shrink, ax=ax)  # need a figure instance for colorbar
...
def _validate_palette(adata: AnnData, key: str) -> None:
    ...
    try:
        ...
        palette = [color if is_color_like(color) else additional_colors[color] for color in raw_palette]
...
def _set_colors_for_categorical_obs(adata, value_to_plot, palette: str | Sequence[str] | Cycler):
    ...
    if isinstance(palette, str) and palette in colormaps:
        ...
        if isinstance(palette, Sequence):
            ...
            try:  # check that colors are valid
                _color_list = [color if is_color_like(color) else additional_colors[color] for color in palette]
            except KeyError as e:
                msg = f"The following color value of the given palette is not valid: {e.args[0]!r}"
...
def plot_arrows(axs, adata, basis, arrows_kwds=None):
    ...
    if v_prefix is None:
        msg = f"`arrows=True` requires `'velocity_{basis}'` from scvelo or `'Delta_{basis}'` from velocyto."
        raise ValueError(msg)
    ...
    *,
    ...
    for icolor, color_spec in enumerate(colors):
        ...
        if colorbars[icolor]:
            ...
            _ = plt.colorbar(sct, format=ticker.FuncFormatter(ticks_formatter), cax=ax_cb)
        # set the title
        if title is not None:
            ax.set_title(title[icolor])
        # output highlighted data points
        for ihighlight, highlight_text in zip(highlights_indices, highlights_labels, strict=True):
