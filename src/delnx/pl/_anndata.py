...
def scatter(
    ...
    *,
    ...
    if _check_if_annotations(adata, "var", x=x, y=y, colors=color, use_raw=use_raw):
        ...
        return axs
    msg = "`x`, `y`, and potential `color` inputs must all come from either `.obs` or `.var`"
    raise ValueError(msg)
...
def _check_if_annotations(
    ...
    *,
    ...
    other_ax_obj = adata.raw if _check_use_raw(adata, use_raw) and axis_name == "obs" else adata
    names: pd.Index[str] = getattr(other_ax_obj, "var" if axis_name == "obs" else "obs").index
    ...
    # Colors are valid
    color_valid: NDArray[np.bool_] = np.fromiter(map(is_color_like, color_idx), dtype=np.bool_, count=len(color_idx))
    ...
    if layers in ["X", None] or (isinstance(layers, str) and layers in adata.layers):
        ...
        for layer in layers:
            if layer not in adata.layers and layer not in ["X", None]:
                msg = "`layers` should have elements that are either None or in adata.layers.keys()."
                raise ValueError(msg)
    else:
        msg = f"`layers` should be a string or a collection of strings with length 3, had value '{layers}'"
        raise ValueError(msg)
        ...
        if groups is None:
            ...
            for name in groups:
                if name not in set(adata.obs[key].cat.categories):
                    msg = f"{name!r} is invalid! specify valid name, one of {adata.obs[key].cat.categories}"
                    raise ValueError(msg)
                else:
...
def clustermap(
    ...
    **kwds
    ...
    if obs_keys is not None:
        ...
        lut = dict(zip(row_colors.cat.categories, adata.uns[obs_keys + "_colors"], strict=True))
...
def heatmap(
    ...
    **kwds
    ...
    """
    var_names, var_groups = _check_var_names_type(var_names, var_group_labels, var_group_positions)
    ...
    var_groups_subset_of_groupby = var_groups is not None and set(var_groups.labels).issubset(categories)
    ...
    if groupby is None or len(categories) <= 1:
        ...
        if isinstance(groupby, str) and isinstance(adata.obs[groupby].dtype, CategoricalDtype):
    ...
    if not swap_axes:
        ...
        if dendrogram:
            ...
            _plot_dendrogram(dendro_ax, adata, groupby, dendrogram_key=_dk(dendrogram), ticks=ticks)
...
def _prepare_dataframe(
    ...
    *,
    ...
    if groupby is None:
        ...
        compound_names = compound_names.cat.reorder_categories(product_order)
