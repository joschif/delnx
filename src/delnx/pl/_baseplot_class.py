...
class BasePlot:
    ...
    def __init__(
        ...
        **kwds,
        ...
        # Ensure groupby is list
        self.groupby = [groupby] if isinstance(groupby, str) else list(groupby)

        # Use _prepare_dataframe to get obs_tidy
        ...
        self.categories = list(self.obs_tidy.index.categories)

        if len(self.categories) > self.MAX_NUM_CATEGORIES:
            warn(
                f"Over {self.MAX_NUM_CATEGORIES} categories found. Plot would be very large.",
                UserWarning,
                stacklevel=2,
            )

        if categories_order is not None and (set(self.obs_tidy.index.categories) != set(categories_order)):
            logg.error(
        ...
        if len(self.groupby) >= 1:
            ...
            self._group_annotation_df = (
                group_df.drop_duplicates("__group__").set_index("__group__").loc[self.obs_tidy.index.categories]
            )
        else:
